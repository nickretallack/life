...
function get_point(array, point) {return array[point[1]][point[0]]}
function set_point(array, point, value) {return array[point[1]][point[0]] = value}
function point_plus(point1, point2) {return [point1[1] + point2[1], point1[0] + point2[0]]}
...
function check_bounds(point, board){
  return point[0] >= 0 && point[1] >= 0 && point[0] < y_size(board) && point[1] < x_size(board)
}
...
function Setup()
{
  ...
  // var maybe_query = maybe_frag[0].split('?')
  var kind = maybe_frag.length > 1 ? maybe_frag[1] : 'blinker'  
  // var size = [40,20]
  // if (maybe_query.length > 1){
  //   var maybe_size = unhash_point(maybe_query[1])
  //   if (maybe_size.length == 2) size = maybe_size
  // }

  setup_board(kind, size)
...
function setup_board(kind, size){
  clearTimeout(timer)
  var board = make_board(size)
  
  $(setups[kind]).each(function(){
